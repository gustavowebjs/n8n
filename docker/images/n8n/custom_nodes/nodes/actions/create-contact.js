"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.createContact = createContact;
async function createContact(context, itemIndex, returnData) {
    const name = context.getNodeParameter("name", itemIndex);
    const email = context.getNodeParameter("email", itemIndex, ""); // Optional with default
    const phone = context.getNodeParameter("phone", itemIndex, ""); // Optional with default
    const response = await context.helpers.httpRequest({
        method: "POST",
        url: "http://localhost:5217/contacts",
        headers: {
            "Content-Type": "application/json",
            Authorization: "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..9f6Ywnzql_CeF_9Q9wivHvefaDjU3SxLrc0GzvjDZKk",
        },
        body: {
            name,
            email,
            phone,
        },
        json: true,
    });
    returnData.push({
        json: response,
    });
}
